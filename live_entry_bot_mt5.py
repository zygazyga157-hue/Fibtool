...
def send_order(
    ...
    """Wrapper to submit an order or log a dry-run.

    Returns a dict-like result. If dry_run True, returns {'retcode':'DRY-RUN','status':'simulated'} and writes a log row to ORDERS_LOG.
    If live, attempts to build a request and call send_order_with_fallback.
    """
    ensure_outputs()
    result = {'retcode': None, 'status': None, 'order': None, 'deal': None, 'comment': None}
    # Normalize side/order kind
    ...
    order_kind_norm = (order_kind or "market").strip().lower()
    if order_kind_norm not in ("market", "limit", "stop"):
        order_kind_norm = "market"

    if dry_run:
        # append to ORDERS_LOG as a DRY-RUN entry
        row = {
            'timestamp': datetime.now(timezone.utc).isoformat(),
            'symbol': symbol,
            'side': side_norm,
            'order_kind': order_kind_norm,
            'price': price,
            'sl': stop,
            'tp': tp,
            'volume': volume,
        ...
        try:
            ...
            try:
                with open(ORDERS_LOG + '.fallback', 'a', encoding='utf-8') as fb:
                    fb.write(json.dumps(row, default=str) + '\n')
            except Exception:
                pass
        result.update({'retcode': 'DRY-RUN', 'status': 'simulated', 'comment': 'dry-run logged'})
        return result
    ...
    # - Buy Stop must be ABOVE current Ask; Sell Stop must be BELOW current Bid
    # If the requested pending kind is invalid, we can fall back to stop/market/skip via env vars.
    pending_invalid_fallback = (os.getenv("MT5_PENDING_INVALID_PRICE_FALLBACK", "stop") or "stop").strip().lower()
    pending_too_close_fallback = (os.getenv("MT5_PENDING_TOO_CLOSE_FALLBACK", "skip") or "skip").strip().lower()
    ...
    def _normalize_price(p: float | None) -> float | None:
        ...
        try:
            ...
            return None
        try:
            return float(_round_price_to_tick(symbol, p))
        except Exception:
            return p

    # Normalize prices to tick size early.
    price_n = _normalize_price(price)
    sl_n = _normalize_price(stop)
    tp_n = _normalize_price(tp)

    # Discover market prices and stops-level constraints (best-effort).
    bid = ask = None
    min_pending_dist = 0.0
    try:
        t = mt5.symbol_info_tick(symbol)
        bid = float(getattr(t, "bid", 0.0) or 0.0) if t is not None else None
        ask = float(getattr(t, "ask", 0.0) or 0.0) if t is not None else None
    except Exception:
        bid = ask = None
    try:
        info = mt5.symbol_info(symbol)
        point = float(getattr(info, "point", 0.0) or 0.0) if info is not None else 0.0
        stops_level = float(getattr(info, "trade_stops_level", 0.0) or 0.0) if info is not None else 0.0
        if point > 0 and stops_level > 0:
            min_pending_dist = stops_level * point
    except Exception:
        min_pending_dist = 0.0

    def _is_pending_favorable(kind: str) -> bool:
        # If we can't fetch bid/ask, assume favorable and let MT5 decide.
        if price_n is None:
            return False
    ...
    def _pending_distance_ok(kind: str) -> bool:
        if min_pending_dist <= 0:
            return True
        if price_n is None:
            return False
        if bid is None or ask is None or bid <= 0 or ask <= 0:
            return True
        ref = ask if side_norm == "long" else bid
        try:
            return abs(float(price_n) - float(ref)) >= float(min_pending_dist)
        except Exception:
            return True

    # Decide final order kind and handle invalid pending placement.
    order_kind_used = order_kind_norm
    if order_kind_used in ("limit", "stop"):
        if not _is_pending_favorable(order_kind_used):
            if pending_invalid_fallback == "stop" and order_kind_used == "limit":
                order_kind_used = "stop"
            elif pending_invalid_fallback == "limit" and order_kind_used == "stop":
                order_kind_used = "limit"
            elif pending_invalid_fallback == "market":
                order_kind_used = "market"
            else:
                return {
                    "retcode": "SKIP",
                    "status": "skipped",
                    "comment": f"Pending {order_kind_norm} invalid vs market (bid={bid} ask={ask} price={price_n}); set MT5_PENDING_INVALID_PRICE_FALLBACK to stop/market",
                }

        # Stops-level/freeze-zone style constraint: pending order too close to current price.
        if order_kind_used in ("limit", "stop") and not _pending_distance_ok(order_kind_used):
            if pending_too_close_fallback == "market":
                order_kind_used = "market"
            else:
                return {
                    "retcode": "SKIP",
                    "status": "skipped",
                    "comment": f"Pending {order_kind_used} too close to market (min_dist={min_pending_dist}); set MT5_PENDING_TOO_CLOSE_FALLBACK=market to force market",
                }

    is_pending = order_kind_used in ("limit", "stop")
    if side_norm == "long":
        if is_pending:
            if order_kind_used == "limit":
                order_type = mt5.ORDER_TYPE_BUY_LIMIT if hasattr(mt5, "ORDER_TYPE_BUY_LIMIT") else mt5.ORDER_TYPE_BUY
            else:
                order_type = mt5.ORDER_TYPE_BUY_STOP if hasattr(mt5, "ORDER_TYPE_BUY_STOP") else mt5.ORDER_TYPE_BUY
        else:
            order_type = mt5.ORDER_TYPE_BUY
    else:
        if is_pending:
            if order_kind_used == "limit":
                order_type = mt5.ORDER_TYPE_SELL_LIMIT if hasattr(mt5, "ORDER_TYPE_SELL_LIMIT") else mt5.ORDER_TYPE_SELL
            else:
                order_type = mt5.ORDER_TYPE_SELL_STOP if hasattr(mt5, "ORDER_TYPE_SELL_STOP") else mt5.ORDER_TYPE_SELL
        else:
            order_type = mt5.ORDER_TYPE_SELL

    if is_pending and hasattr(mt5, "TRADE_ACTION_PENDING"):
        action = mt5.TRADE_ACTION_PENDING
    else:
        action = mt5.TRADE_ACTION_DEAL

    # For pending orders prefer RETURN (when available) to reduce rejects; otherwise fall back.
    if is_pending and hasattr(mt5, "ORDER_FILLING_RETURN"):
        filling = mt5.ORDER_FILLING_RETURN
    else:
        filling = mt5.ORDER_FILLING_FOK if hasattr(mt5, 'ORDER_FILLING_FOK') else 0

    # Build request - conservative defaults
    req = {
        'action': action,
        'symbol': symbol,
        'type': order_type,
        'volume': float(volume),
        'price': float(price_n) if price_n is not None else None,
        'sl': float(sl_n) if sl_n is not None else 0.0,
        'tp': float(tp_n) if tp_n is not None else 0.0,
        'type_time': mt5.ORDER_TIME_GTC if hasattr(mt5, 'ORDER_TIME_GTC') else 0,
        'type_filling': filling,
        'comment': comment or 'asia_sweep'
    }
    try:
        res = send_order_with_fallback(symbol, req)
        raw = res.get('result') if isinstance(res, dict) else res
        sanitized_comment = res.get('sanitized_comment') if isinstance(res, dict) else getattr(raw, 'comment', None)
        # normalize minimal info
        retcode = getattr(raw, 'retcode', None)
        ret = {
            'retcode': retcode,
            'status': 'sent' if retcode in (0, 10009) else 'failed',
            'order': getattr(raw, 'order', None),
            'deal': getattr(raw, 'deal', None),
            'comment': sanitized_comment,
            'order_kind_used': order_kind_used,
        }
        # log to ORDERS_LOG
        row = {
            'timestamp': datetime.now(timezone.utc).isoformat(),
            ...
            'tp': tp,
            'volume': volume,
            'result_retcode': ret['retcode'],
            'order': ret.get('order'),
            'deal': ret.get('deal'),
            'original_comment': comment,
            'sanitized_comment': sanitized_comment,
            'comment': str(ret.get('status'))
        }
        try:
            with open(ORDERS_LOG, 'a', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=list(row.keys()))
...
def fetch_bars(symbol: str, tf, count: int = 500) -> pd.DataFrame:
    ...
    # treat MT5 returned epoch seconds as UTC
...
def ensure_execute_order(request: dict, max_retries: int = 3, backoff_seconds: float = 1.0):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_result = mt5.order_send(safe_req)
...
def send_order_with_fallback(symbol: str, request: dict, max_retries: int = 3, backoff_seconds: float = 1.0):
    """Compatibility wrapper used by send_order() live path.

    Historically other modules called `send_order_with_fallback(symbol, request)`.
    This project now centralizes retry + comment sanitization in `ensure_execute_order`.
    """
    try:
        # Best-effort: ensure the symbol is selected/visible for MT5 before sending.
        if mt5 is not None:
            try:
        ...
        pass
    return ensure_execute_order(request, max_retries=max_retries, backoff_seconds=backoff_seconds)
