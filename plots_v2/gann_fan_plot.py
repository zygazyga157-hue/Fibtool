    ...
    for sym in cfg.symbols:
        for tf in cfg.timeframes:
            ...
            spec = MT5ObjectSpec(
                ...
                anchor_2=a2,
                strength=0.6,
                context=ObjectContext(label="gann_fan", sources=[f"anchor:{sel.kind}"]),
                metadata={
                    **build_description_metadata(
                        "gann_fan",
                        score=0.6,
                        anchor_source=sel.kind,
                        components=["pivot", "price_time_scale", unit_mode],
                    ),
                    "scale": scale,
            ...
            update_chart_metrics(outdir, specs_created=[spec], lifecycle_state=lifecycle, run_label="gann_fan_v2")

            if cfg.emit_spec_only:
                print(f"[V2][FAN] Spec only: {sym} {tf} id={spec.object_id}")
                continue
            ...
            print(f"[V2][FAN] Wrote {apply_path.name} + {cleanup_path.name} for {sym} {tf}")


def main() -> None:
    p = build_parser(description="Plots V2 Tool 3 — Gann Fan (OBJ_GANNFAN)")
